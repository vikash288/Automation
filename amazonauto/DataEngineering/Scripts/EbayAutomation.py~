...
def upload_to_s3(aws_access_key_id, aws_secret_access_key, file, bucket, key, callback=None, md5=None, reduced_redundancy=False, content_type=None):
    ...
    logger.info('file uploaded in bucket: '+bucket+' key: '+key)
...
def get_ebay_data(date,category,path):
    ...
    logger.info("Request to ebay API "+date+" response code "+response.status_code)
    # this is gz data
...
def truncate_target_table(db,target_table_name,target_previous_table_name):
    ...
    movedata = "insert into "+target_previous_table_name+" select *, now() as TIMEAUDIT from "+target_table_name;
...
def load_data_into_nonupc_table(db,target_table_name,target_previous_table_name,not_upc_match_table_name):
    ...
    nonmatchupc = "insert into "+not_upc_match_table_name+" SELECT SRC.*,now() as TIMEAUDIT FROM "+target_previous_table_name+" SRC LEFT OUTER JOIN "+target_table_name+" TGT ON SRC.UPC = TGT.UPC WHERE TGT.UPC IS NULL"
...
with open("ebaytables.csv") as csvfile:
    ...
    mailsubject="ebay daily load complete - test"
