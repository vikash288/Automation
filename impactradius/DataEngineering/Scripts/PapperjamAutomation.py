...
def deleteauto(loadtype,jarpath):
  with open("pepperjamsource.csv") as csvfile:
...
with open("pepperjamsource.csv") as csvfile:
    ...
    for row in readCSV:
        ...
        if(loadtype=="full" and fullload_flag=="1" and s3flag=="1" and mysqlflag=="1" and esflag=="1"):
            subprocess.call(['java', '-jar', '-Xmx512m', jarpath, 'pjauto', ftphostname, ftpusername, ftppassword, port, foldername, sourcefile, retailer, affiliator, delimiter,retailercode,loadtype,s3flag,mysqlflag,esflag])
        elif(loadtype=="incremental" and incremental_flag=="1" and s3flag=="1" and mysqlflag=="1" and esflag=="1"):
            subprocess.call(['java', '-jar', '-Xmx512m', jarpath, 'pjauto', ftphostname, ftpusername, ftppassword, port, foldername, incrementalfile, retailer, affiliator, delimiter,retailercode,loadtype,s3flag,mysqlflag,esflag])
        else:
            print("Execution Halted, Please Add proper parameter value on the CSV file mention the load type and process that you want to execute")
