...
public class PapperjamAutomation {
	...
	// Add logger instance
	Logger logger = Logger.getLogger(PapperjamAutomation.class);
	...
	public void automate(String hostName, String userName, String password, int port, String sourceFolderName,
			...
			int s3flag, int mysqlflag, int esflag) throws IOException {
		...
		try {
			...
			if (s3flag == 1) {
				...
				if (affiliator.equalsIgnoreCase("papperjam")
						&& (loadtype.equalsIgnoreCase("incremental") || loadtype.equalsIgnoreCase("full"))) {

					// FTP File Download
					sfile = ftp.httpDownloadFile(hostName, retailer, prop.getDumpfilepath(),logFileName);
					...
					if (sfile.isEmpty()) {
						...
						mail.sendMailGet(
								affiliator + ":" + retailer.substring(0, 1).toUpperCase()
										+ retailer.substring(1).toLowerCase() + ": " + loadtype + " File size is zero",
								mailBody);
					}
			...
			if (mysqlflag == 1 && !sfile.isEmpty()) {
				 ...
				 * mysqlOpp.deletePartialRecord(conn, targetTableName,
				 * retailerno,
				 * util.cleanSpecialCharacter(retailer).toLowerCase(),
				 * logFileName); }
				 */
			}
		...
		//Exit System once one reailer done 
		System.exit(0);
	}
	...
	public static void main(String args[]) throws IOException {
		PapperjamAutomation auto = new PapperjamAutomation();
		 ...
		 * "38733_3310515_mp.txt.gz"; String retailer = "Sam's Club"; String
		 * affiliator = "LinkShare"; String delimiter = "|"; String retailerno =
		 * "25";
		 */

		String hostName = "https://s3.amazonaws.com/price.com-affiliatefeed/CostumeKingdom.com/CostumeKingdomproducts.csv";
		String userName = "";
		String password = "";
		int port = 21;
		String sourceFolderName = "";
		String sourceFile = "";
		String retailer = "CostumeKingdom.com";
		String affiliator = "papperjam";
		String delimiter = ",";
		String retailerno = "73";
