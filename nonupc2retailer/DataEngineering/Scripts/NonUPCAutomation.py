...
with open("nonupcsources.csv") as csvfile:
    ...
    for row in readCSV:
        ...
        if(loadtype=="full" and fullload_flag=="1" and s3flag=="1" and mysqlflag=="1" and esflag=="1"):
            subprocess.call(['java', '-jar', '-Xmx512m', jarpath, 'nonupc2retailer', ftphostname, ftpusername, ftppassword, port, foldername, sourcefile, retailer, affiliator, delimiter,retailercode,loadtype,s3flag,mysqlflag,esflag,stg_table_name])
        elif(loadtype=="incremental" and incremental_flag=="1" and s3flag=="1" and mysqlflag=="1" and esflag=="1"):
            subprocess.call(['java', '-jar', '-Xmx512m', jarpath, 'nonupc2retailer', ftphostname, ftpusername, ftppassword, port, foldername, incrementalfile, retailer, affiliator, delimiter,retailercode,loadtype,s3flag,mysqlflag,esflag,stg_table_name])
        else:
